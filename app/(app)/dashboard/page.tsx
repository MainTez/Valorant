...
export default async function DashboardPage() {
  ...
  const recentResults = ((recentMatches ?? []) as MatchRow[]).map((m) => m.result);
  ...
  const activity = ((activityRows ?? []) as ActivityEventRow[]).map((a) => ({
    ...a,
  ...
  return (
    <div className="flex flex-col gap-5 max-w-[1400px]">
      <header>
        <div className="font-display tracking-wide text-4xl">
          Welcome back,{" "}
          <span className="accent-text">
            {user.display_name ?? user.email.split("@")[0]}
          </span>
        </div>
        <p className="text-[color:var(--color-muted)] mt-1">Let&rsquo;s win today.</p>
        <div className="h-[2px] w-16 bg-[color:var(--accent)] mt-3 rounded-full shadow-[0_0_10px_var(--accent)]" />
      </header>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1fr_1.3fr_1fr] gap-4">
...
function buildWinTrend(results: Array<"win" | "loss" | "draw">): number[] {
  // Rolling cumulative win-rate over the provided sequence (newest first)
  ...
  for (const r of ordered) {
    ...
    out.push(decided === 0 ? 0 : wins / decided);
  }
  return out;
}
